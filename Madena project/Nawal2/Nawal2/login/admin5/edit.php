...
if(count($_POST)>0) {
mysqli_query($conn,"UPDATE regdata5 set id='" . $_POST['id'] . "', Name='" . $_POST['Name'] . "', Age='" . $_POST['Age'] . "', Country='" . $_POST['Country'] . "', Phone='" . $_POST['Phone'] . "' , Address='" . $_POST['Address'] . "' , QuranKnow='" . $_POST['QuranKnow'] . "' , Level='" . $_POST['Level'] . "'  , Drash='" . $_POST['Drash'] . "' , transp='" . $_POST['transp'] . "' , Notes='" . $_POST['Notes'] . "' WHERE id='" . $_POST['id'] . "'");
$message = "<div class='alert alert-success'>
تم التعديل بنجاح
</div>";
}
$result = mysqli_query($conn,"SELECT * FROM regdata5 WHERE id='" . $_GET['id'] . "'");
...
<!--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
        <h3><label >نوع الإعاقة : </label></h3>
        <select name="a7aqh" class="form-control input-lg" >
                <option   value="نوع الإعاقة">نوع الإعاقة</option>
				<option value="بصرية"
        <?php if($row["a7aqh"]=='بصرية') {
          echo "selected";
        } ?>    >بصرية</option>
				
        <option value="سمعية"
        <?php if($row["a7aqh"]=='سمعية') {
          echo "selected";
        } ?>  >سمعية</option>

        <option value="حركية" 
        <?php if($row["a7aqh"]=='حركية') {
          echo "selected";
        } ?>  >حركية</option>
